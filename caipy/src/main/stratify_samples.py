...
class stratification:
    def main(self, sysargs = None):
        parser = argparse.ArgumentParser()
        parser.add_argument('-m', '--meta_name', type=str, default=None, help='Metadata filename FULL path')
        ...
        if meta_path is None:
            meta_path = utilities.dataLoadingUtilities.check_csv(input('Path location and file name: (e.g. C:/Documents/meta_file.csv) '))
            while not os.path.exists(meta_path):
                print(f'Error: path {meta_path} does not exist')
                meta_path = utilities.dataLoadingUtilities.check_csv(input('Path location and file name: (e.g. C:/Documents/meta_file.csv) '))
        else: print("Received metadata file path: ", meta_path)
        if n_folds is None:
        ...
        meta = pd.read_csv(meta_path).reset_index()
        has_sample = True if 'Sample_Name' in meta.columns else False
        if var_list is None: var_list = [c for c in meta.columns if c not in ['index', 'pkey', 'Sample_Name']]
        if len(var_list) > 1:
            # make sum column
        ...
        #folder = '\\'.join(meta_path.split('\\')[:-1])
        filename = meta_path.split('\\')[-1][:-4]
        new_filename = f'{filename}_stratified.csv'
        print("filename location", new_filename)
        meta.to_csv(f'{new_filename}', index=False)
        print('Exported', new_filename)
        Globals.meta_path = os.path.join(Globals.data_folder, new_filename)
        return meta
    
#main method called for purposes of poetry execution
